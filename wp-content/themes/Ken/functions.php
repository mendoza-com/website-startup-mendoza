...
class Theme {
	...
	function supports() {
		...
		if (!isset($content_width)) {$content_width = $mk_settings['grid-width'];
		}
		...
		if (function_exists('add_theme_support')) {
			...
			/* Add Woocmmerce support and disable woocmmrece prettyphoto script and style to prevent conflicts */
...
function theme_enqueue_scripts() {
	if (!is_admin()) {
		...
		wp_register_script('instafeed', THEME_JS . '/instafeed.min.js', array('jquery'), $theme_data['Version'], true);
		wp_enqueue_script('skrollr', THEME_JS . '/skrollr-min.js', array('jquery'), $theme_data['Version'], true);

		if ($mk_settings['smooth-scroll']) {
			wp_enqueue_script('smoothScroll', THEME_JS . '/jquery.nicescroll.js', array('jquery'), $theme_data['Version'], true);
		}

		if ($mk_settings['smooth-scroll'] == 1) {
			wp_enqueue_script('SmoothScroll', THEME_JS . '/SmoothScroll.js', array('jquery'), $theme_data['Version'], true);
		}

		if ($mk_settings['minify-js']) {
			wp_enqueue_script('theme-plugins-min', THEME_JS . '/min/plugins-ck.js', array('jquery'), $theme_data['Version'], true);
			wp_enqueue_script('theme-scripts-min', THEME_JS . '/min/theme-scripts-ck.js', array('jquery'), $theme_data['Version'], true);
		} else {
			wp_enqueue_script('theme-plugins', THEME_JS . '/plugins.js', array('jquery'), $theme_data['Version'], true);
		...
		if(file_exists($custom_js_file)) {
			wp_enqueue_script('custom-js', $custom_js_file_uri, array('jquery'), $theme_data['Version'], true);	
		}
		...
		wp_enqueue_style('theme-styles', THEME_STYLES . '/theme-styles'.$css_min.'.css', false, $theme_data['Version'], 'all');
		wp_enqueue_style('artbees-icons', THEME_STYLES . '/artbees-icons'.$css_min.'.css', false, $theme_data['Version'], 'all');
		wp_enqueue_style('fontawesome', THEME_STYLES . '/font-awesome'.$css_min.'.css', false, $theme_data['Version'], 'all');
		wp_enqueue_style('pe-line-icons', THEME_STYLES . '/pe-line-icons'.$css_min.'.css', false, $theme_data['Version'], 'all');
		wp_enqueue_style('flaticon', THEME_STYLES . '/flaticon'.$css_min.'.css', false, $theme_data['Version'], 'all');
...
function mk_preloader_script() {
	...
	if (!global_get_post_id()) {return false;}

	$preloader = get_post_meta(global_get_post_id(), '_preloader', true);

	if ($preloader == 'true') {
		wp_enqueue_script('QueryLoader', THEME_JS . '/jquery.queryloader2-min.js', array('jquery'), false, false);
	}
