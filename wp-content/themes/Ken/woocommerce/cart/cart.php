...
<table class="shop_table cart" cellspacing="0">
	<thead>
		<tr>
			...
			<th class="product-price"><?php _e( 'Price', 'mk_framework' ); ?></th>
			<th class="product-quantity"><?php _e( 'Quantity', 'mk_framework' ); ?></th>
			<th class="product-subtotal"><?php _e( 'Total', 'mk_framework' ); ?></th>
		</tr>
	</thead>
	<tbody>
			...
			if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
				...
				<tr class="<?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
					...
					<td class="product-remove">
						<?php
							echo apply_filters( 'woocommerce_cart_item_remove_link', sprintf( '<a href="%s" class="remove" title="%s"><i class="mk-icon-close"></i></a>', esc_url( WC()->cart->get_remove_url( $cart_item_key ) ), __( 'Remove this item', 'mk_framework' ) ), $cart_item_key );
						?>
					</td>
						...
						<?php
							...
							echo WC()->cart->get_item_data( $cart_item );
               				...
               				if ( $_product->backorders_require_notification() && $_product->is_on_backorder( $cart_item['quantity'] ) )
               					echo '<p class="backorder_notification">' . __( 'Available on backorder', 'mk_framework' ) . '</p>';
						?>
					</td>
