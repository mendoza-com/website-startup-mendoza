...
function blog_masonry_style( $atts) {
	...
	$blog_heading .= '<h2 class="blog-title"><a href="'.get_permalink().'">'.get_the_title().'</a></h2>';
			if($column != 4) {
				...
				$blog_heading .= '<a href="'.get_permalink().'#comments" class="blog-comments"><i class="mk-icon-comment"></i>'.ob_get_clean().'</a>';

				if( function_exists('mk_love_this') ) {
					ob_start();
					mk_love_this();
					$blog_heading .= '<div class="mk-love-holder">'.ob_get_clean().'</div>';
				}
			}
	$blog_heading .= '</div>';
	/***********/
	...
	switch ($post_type) {
		...
		if ( has_post_thumbnail() ) {
			$output .='<div class="featured-image">';
				...
				$output .= '<a title="'.get_the_title().'" href="'.get_permalink().'"><i class="mk-theme-icon-next-big hover-plus-icon-xsmall"></a></i>';
			$output .='</div>';
		}
			break;
		/***********/
