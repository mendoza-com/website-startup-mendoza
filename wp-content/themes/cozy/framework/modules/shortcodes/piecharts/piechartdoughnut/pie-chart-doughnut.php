<?php
namespace CozyEdge\Modules\Shortcodes\PieCharts\PieChartDoughnut;
...
class PieChartDoughnut implements ShortcodeInterface {
	...
	public function __construct() {
		$this->base = 'edgtf_pie_chart_doughnut';
	...
	public function vcMap() {
		...
		for ($i = 1; $i <= $this->chartFields; $i++) {
			...
			$chart_fields[] = array(
				'type' => 'textfield',
				'heading' => esc_html__('Chart Value ', 'cozy') . $i,
				'param_name' => "chart_value_" . $i,
			);
			$chart_fields[] = array(
				'type' => 'colorpicker',
				'heading' => esc_html__('Chart Color ', 'cozy') . $i,
				'param_name' => "chart_color_" . $i,
			);
			$chart_fields[] = array(
				'type' => 'textfield',
				'heading' => esc_html__('Chart Legend ', 'cozy') . $i,
		...
		vc_map( array(
			'name' => esc_html__('Edge Pie Chart 3 (Doughnut)', 'cozy'),
			'base' => $this->getBase(),
			...
			'params' => array_merge(
				array(
					...
					array(
						'type' => 'textfield',
						'heading' => esc_html__('Height','cozy'),
						'param_name' => 'height',
					),
				),
				$chart_fields
	...
	public function render($atts, $content = null) {
		...
		$html = cozy_edge_get_shortcode_module_template_part('templates/pie-chart-doughnut', 'piecharts/piechartdoughnut', '', $params);
