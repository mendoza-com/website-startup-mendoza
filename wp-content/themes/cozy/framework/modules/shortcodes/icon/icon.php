<?php
namespace CozyEdge\Modules\Shortcodes\Icon;

use CozyEdge\Modules\Shortcodes\Lib\ShortcodeInterface;

/**
 * Class Icon
 * @package CozyEdge\Modules\Shortcodes\Icon
 */
class Icon implements ShortcodeInterface {
    ...
    public function vcMap() {
        vc_map(array(
            'name'                      => esc_html__('Edge Icon', 'cozy'),
            'base'                      => $this->base,
            ...
            'params'                    => array_merge(
                ...
                array(
                    ...
                    array(
                        'type'        => 'textfield',
                        'admin_label' => true,
                        'heading'     => esc_html__('Custom Size (px)', 'cozy'),
                    ...
                    array(
                        'type'        => 'textfield',
                        'heading'     => esc_html__('Border radius', 'cozy'),
                        'param_name'  => 'border_radius',
                        'description' => esc_html__('Please insert border radius(Rounded corners) in px. For example: 4 ', 'cozy'),
                    ...
                    array(
                        ...
                        'admin_label' => true,
                        'description' => esc_html__('Enter just number. Omit pixels', 'cozy'),
                        'dependency'  => array('element' => 'type', 'value' => array('circle', 'square'))
                    ),
                    array(
                        'type'        => 'colorpicker',
                    ...
                    array(
                        'type'        => 'colorpicker',
                        'heading'     => esc_html__('Hover Icon Color', 'cozy'),
                    ...
                    array(
                        ...
                        'param_name'  => 'margin',
                        'admin_label' => true,
                        'description' => esc_html__('Margin (top right bottom left)', 'cozy'),
                    ),
                    array(
                        'type'        => 'dropdown',
                    ...
                    array(
                        'type'        => 'checkbox',
                        'heading'     => esc_html__('Use Link as Anchor', 'cozy'),
                        'value'       => array('Use this icon as Anchor?' => 'yes'),
                        'param_name'  => 'anchor_icon',
                        'admin_label' => true,
                        'description' => esc_html__('Check this box to use icon as anchor link (eg. #contact)', 'cozy'),
    ...
    public function render($atts, $content = null) {
        ...
        $html = cozy_edge_get_shortcode_module_template_part('templates/icon', 'icon', '', $params);
        ...
        if(($params['type'] !== 'normal' && !empty($params['shape_size'])) ||
           ($params['type'] == 'normal')
            ...
            if(!empty($params['custom_size'])) {
                $iconStyles[] = 'font-size:'.cozy_edge_filter_px($params['custom_size']).'px';
            }
        }
    ...
    private function generateIconHolderStyles($params) {
        ...
        if(isset($params['type']) && $params['type'] !== 'normal') {
            ...
            if(!empty($shapeSize)) {
                $iconHolderStyles[] = 'width: '.cozy_edge_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'height: '.cozy_edge_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'line-height: '.cozy_edge_filter_px($shapeSize).'px';
            }
            ...
            if(!empty($params['border_color']) && (isset($params['border_width']) && $params['border_width'] !== '')) {
				...
				$iconHolderStyles[] = 'border-width: '.cozy_edge_filter_px($params['border_width']).'px';
            ...
            if($params['type'] == 'square') {
                if(isset($params['border_radius']) && $params['border_radius'] !== '') {
                    $iconHolderStyles[] = 'border-radius: '.cozy_edge_filter_px($params['border_radius']).'px';
                }
            }
        }
