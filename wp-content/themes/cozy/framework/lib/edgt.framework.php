...
class CozyEdgeFramework {
    ...
    private function __construct() {
        $this->edgtOptions = CozyEdgeOptions::get_instance();
        $this->edgtMetaBoxes = CozyEdgeMetaBoxes::get_instance();
    }
    ...
    public function setSkin(CozyEdgeSkinAbstract $skinObject) {
        $this->skin = $skinObject;
    }
}

/**
 * Class CozyEdgeSkinManager
 *
...
/**
 * Class CozyEdgeSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class CozyEdgeSkinAbstract {
    /**
     * @var string
     */
    protected $skinName;
    ...
    public function loadTemplatePart($template, $params = array()) {
        ...
        if($template !== '') {
            include(cozy_edge_find_template_path('framework/admin/skins/'.$this->skinName.'/templates/'.$template.'.php'));
        }
    }
    ...
    public function getMenuIcon($icon) {
        ...
        return cozy_edge_get_skin_uri().'/assets/img/admin-logo-icon.png';
    }
    ...
    public function setShortcodeJSParams() { ?>
        <script>
            window.edgtSCIcon = '<?php echo cozy_edge_get_skin_uri().'/assets/img/admin-logo-icon.png'; ?>';
...
class CozyEdgeOptions {
    ...
    public function getOptionValue($key) {
        ...
        if(array_key_exists($key, $cozy_edge_options)) {
            return $cozy_edge_options[$key];
...
/*
   Class: CozyEdgeAdminPage
   A class that initializes Edge Admin Page
*/
class CozyEdgeAdminPage implements iCozyEdgeLayoutNode {
    ...
    function __construct($slug="", $title="", $icon = "") {
        $this->layout = array();
        $this->factory = new CozyEdgeFieldFactory();
        $this->slug = $slug;
        $this->title = $title;
        $this->icon = $icon;
    }
    ...
    public function renderChild(iCozyEdgeRender $child) {
        $child->render($this->factory);
    }
}

/*
   Class: CozyEdgeMetaBoxes
...
/*
   Class: CozyEdgeMetaBox
   A class that initializes Edge Meta Box
*/
class CozyEdgeMetaBox implements iCozyEdgeLayoutNode {
    ...
    public function setTitle($label) {
        ...
        return $cozy_edge_Framework->getSkin()->getSkinLabel().' '.$label;
    }
}

global $cozy_edge_Framework;

$cozy_edge_Framework = CozyEdgeFramework::get_instance();
$edgtSkinManager = new CozyEdgeSkinManager();
$cozy_edge_Framework->setSkin($edgtSkinManager->getSkin());
