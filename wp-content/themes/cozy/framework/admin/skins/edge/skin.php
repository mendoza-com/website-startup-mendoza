...
class EdgeSkin extends CozyEdgeSkinAbstract {
    /**
     * Skin constructor. Hooks to cozy_edge_admin_scripts_init and cozy_edge_enqueue_admin_styles
     */
    public function __construct() {
        $this->skinName = 'edge';

        //hook to
        add_action('cozy_edge_admin_scripts_init', array($this, 'registerStyles'));
        add_action('cozy_edge_admin_scripts_init', array($this, 'registerScripts'));

        add_action('cozy_edge_enqueue_admin_styles', array($this, 'enqueueStyles'));
        add_action('cozy_edge_enqueue_admin_scripts', array($this, 'enqueueScripts'));

        add_action('cozy_edge_enqueue_meta_box_styles', array($this, 'enqueueStyles'));
        add_action('cozy_edge_enqueue_meta_box_scripts', array($this, 'enqueueScripts'));
    ...
    public function registerScripts() {
        ...
        foreach ($this->scripts as $scriptHandle => $scriptPath) {
            cozy_edge_register_skin_script($scriptHandle, $scriptPath);
        }
    }
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
            cozy_edge_register_skin_style($styleHandle, $stylePath);
        }
    ...
    public function renderOptions() {
        ...
        $tab    = cozy_edge_get_admin_tab();
        $active_page = $cozy_edge_Framework->edgtOptions->getAdminPageFromSlug($tab);
     ...
     *
*@param CozyEdgeAdminPage $page current page to load
     *
     * @see EdgeSkinAbstract::loadTemplatePart()
     */
