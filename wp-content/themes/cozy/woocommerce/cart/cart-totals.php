...
<div class="cart_totals <?php if ( WC()->customer->has_calculated_shipping() ) echo 'calculated_shipping'; ?>">
	...
	<h3><?php esc_html_e( 'Cart Total', 'cozy' ); ?></h3>

	<div class="edgtf-total-pricing clearfix">

		<div class="cart-subtotal">
			<h4><?php esc_html_e( 'Subtotal:', 'cozy' ); ?></h4>
			<span data-title="<?php esc_attr_e( 'Subtotal', 'cozy' ); ?>"><?php wc_cart_totals_subtotal_html(); ?></span>
		</div>
		...
		<?php elseif ( WC()->cart->needs_shipping() ) : ?>
			...
			<div class="shipping">
				<h4><?php esc_html_e( 'Shipping:', 'cozy' ); ?></h4>
				<span data-title="<?php esc_attr_e( 'Shipping', 'cozy' ); ?>"><?php woocommerce_shipping_calculator(); ?></span>
			</div>
...
<?php if ( wc_tax_enabled() && 'excl' === WC()->cart->tax_display_cart ) :
			...
			$estimated_text  = WC()->customer->is_customer_outside_base() && ! WC()->customer->has_calculated_shipping()
					? sprintf( ' <small>(' . __( 'estimated for %s', 'cozy' ) . ')</small>', WC()->countries->estimated_for_prefix( $taxable_address[0] ) . WC()->countries->countries[ $taxable_address[0] ] )
					: '';
		...
		<div class="order-total">
			<h4><?php _e( 'Total:', 'cozy' ); ?></h4>
			<span data-title="<?php esc_attr_e( 'Total', 'cozy' ); ?>"><?php wc_cart_totals_order_total_html(); ?></span>
		</div>
