...
if( ! empty($row_type) && $row_type == 'parallax'){
	...
	if(cozy_edge_options()->getOptionValue('parallax_on_off') == 'off'){
		$css_classes[] = 'edgtf-parallax-section-holder-touch-disabled';
	}
	if($parallax_speed != ''){
...
if($row_type == 'row' && $css_animation != ''){
	...
	if($transition_delay !== ''){
		...
		$inner_start .= '<div '.cozy_edge_get_inline_style($animation_styles).'>';
		$inner_end .= '</div>';
	}else{
		$inner_start .= '<div>';
		$inner_end .= '</div>';
...
if($header_style != ''){
	$wrapper_attributes[] = 'data-edgtf_header_style="'.$header_style.'"';
}
...
if($video == 'show_video'){
	...
	$video_overlay_class = 'edgtf-video-overlay';
	$video_overlay_style = '';
	$video_mobile_style = '';
	$video_attrs = '';
	$v_image = '';
	if($video_overlay == "show_video_overlay"){
		$video_overlay_class .= ' edgtf-video-overlay-active';
	}
	if($video_image) {
	...
	if($video_image) {
		$video_output .= '<div class="edgtf-mobile-video-image" ' . cozy_edge_get_inline_attr($video_mobile_style, 'style') . '></div>';
	}
	$video_output .= '<div ' . cozy_edge_get_class_attribute($video_overlay_class) . cozy_edge_get_inline_attr($video_overlay_style, 'style') . '></div>';
	$video_output .= '<div class="edgtf-video-wrap">';
	$video_output .= '<video class="edgtf-video" width="1920" height="800" '. cozy_edge_get_inline_attr($video_attrs, 'poster')  .' controls="controls" preload="auto" loop autoplay muted>';
	...
	if($v_image) {
		$video_output .= '<img ' . cozy_edge_get_inline_attr($v_image, 'src') . ' width="1920" height="800" title="No video playback capabilities" alt="'.esc_html__('video thumb','cozy').'" />';
	}
	$video_output .='</object>';
	$video_output .='</video>';
	$video_output .='</div>';
