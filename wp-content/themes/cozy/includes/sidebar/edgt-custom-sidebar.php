...
if( ! class_exists('CozyEdgeSidebar') ){
	
	class CozyEdgeSidebar{
		...
		function __construct(){
			...
			$this->title = esc_html__('Custom Widget Area','cozy');
		...
		function template_add_widget_field(){
			...
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '".esc_html__('Enter Name of the new Widget Area', 'cozy')."' name='edgt-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='".esc_html__('Add Widget Area', 'cozy')."' />";
			echo "\n    ".$nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function add_sidebar_area(){
			if(!empty($_POST['edgt-add-widget'])){
				...
				$name = rtrim($this->get_name($_POST['edgt-add-widget']));
		...
		function register_custom_sidebars(){
			...
			$args = apply_filters('cozy_edge_custom_widget_args', $args);
