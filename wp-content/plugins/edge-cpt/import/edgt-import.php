...
class CozyEdgeImport {
    ...
    public function import_options($file){
        $options = $this->file_options($file);
        update_option( 'edgt_options_cozy', $options);
    ...
    function edgt_admin_import() {
        if (edgt_core_theme_installed()) {
            ...
            $this->pagehook = add_submenu_page(
                ...
                'cozy_edge_theme_menu'.$slug,                // The ID used to bind submenu items to this menu
                array($cozy_edge_Framework->getSkin(), 'renderImport')
            );

            add_action('admin_print_scripts-'.$this->pagehook, 'cozy_edge_enqueue_admin_scripts');
            add_action('admin_print_styles-'.$this->pagehook, 'cozy_edge_enqueue_admin_styles');
...
function edgt_init_import_object(){
    global $cozy_edge_import_object;
    $cozy_edge_import_object = new CozyEdgeImport();
}

add_action('init', 'edgt_init_import_object');

if(!function_exists('cozy_edge_dataImport')){
    function cozy_edge_dataImport(){
        global $cozy_edge_import_object;

        if ($_POST['import_attachments'] == 1)
            $cozy_edge_import_object->attachments = true;
        else
            $cozy_edge_import_object->attachments = false;

        $folder = "cozy/";
        if (!empty($_POST['example']))
            $folder = $_POST['example']."/";

        $cozy_edge_import_object->import_content($folder.$_POST['xml']);

        die();
    }

    add_action('wp_ajax_edgt_dataImport', 'cozy_edge_dataImport');
}

if(!function_exists('cozy_edge_widgetsImport')){
    function cozy_edge_widgetsImport(){
        global $cozy_edge_import_object;
        ...
        $cozy_edge_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');

        die();
    }

    add_action('wp_ajax_edgt_widgetsImport', 'cozy_edge_widgetsImport');
}

if(!function_exists('cozy_edge_optionsImport')){
    function cozy_edge_optionsImport(){
        global $cozy_edge_import_object;
        ...
        $cozy_edge_import_object->import_options($folder.'options.txt');

        die();
    }

    add_action('wp_ajax_edgt_optionsImport', 'cozy_edge_optionsImport');
}

if(!function_exists('cozy_edge_otherImport')){
    function cozy_edge_otherImport(){
        global $cozy_edge_import_object;
        ...
        $cozy_edge_import_object->import_options($folder.'options.txt');
        $cozy_edge_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');
        $cozy_edge_import_object->import_menus($folder.'menus.txt');
        $cozy_edge_import_object->import_settings_pages($folder.'settingpages.txt');

        die();
    }

    add_action('wp_ajax_edgt_otherImport', 'cozy_edge_otherImport');
}