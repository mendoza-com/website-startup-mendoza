...
function bawsi_do_invit() {
	...
	if ( empty( $message ) && ! $token ) {
		...
		if ( count( $groups ) === 1 ) {
			$message .= sprintf( __( 'Ingresá tu correo electrónico para unirte a <a href="http://%1$s.slack.com"><b>%1$s</b></a> en Slack.', 'bawsi' ), esc_html( key( $groups ) ) );
		} else {
	...
	if ( ! empty( $_POST['email'] ) && isset( $group ) && is_email( $_POST['email'] ) ) {
		...
		if( ! is_wp_error( $response ) && 200 === wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( $return->ok ) {
				...
				$message = $dom . sprintf( __( 'Invitación enviada, gracias.<br>Te leemos pronto en <a href="%1$s">%2$s</a>.<br><i>- %3$s</i>', 'bawsi' ), $slack_url, esc_html( $group ), $botname );
			} 
			if ( isset( $return->error ) ) {
				switch( $return->error ) {
					case 'already_in_team' :
						$message = $dom . __( '¡Ya estás en el grupo de StartUp Mendoza!', 'bawsi' );
					break;
					case 'already_invited' :
					case 'sent_recently' :
						$message = $dom . __( '¡Te enviamos una invitación a tu correo para que te sumes a StartUp Mendoza!', 'bawsi' );
					break;					
					case 'invalid_auth' :
	...
	?>
		...
		<form action="" method="post">
			...
			<p><input type="submit" value="<?php esc_attr_e( 'Enviar invitación', 'bawsi' ); ?>" name="submit" id="submit" class="button button-primary"/></p>
		</form>
	<?php
